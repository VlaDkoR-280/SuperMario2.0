...
void draw_map(unsigned i_view_x, const sf::Image& i_map_sketch, sf::RenderWindow& i_window, const sf::Texture& i_map_texture, const Map& i_map)
{
	...
	for (unsigned short a = map_start; a < map_end; a++)
	{
		for (unsigned short b = 0; b < map_height; b++)
		{
			...
			sf::Color pixel = i_map_sketch.getPixel(a, b + 2 * map_height - 1);
			...
			if (255 == pixel.a)
			{
				...
				if (0 < a)
				{
					pixel_left = i_map_sketch.getPixel(a - 1, b + 2 * map_height - 1);
				}
				...
				{
					pixel_right = i_map_sketch.getPixel(1 + a, b + 2 * map_height - 1);
				}

				if (b < map_height - 1)
				{
					pixel_down = i_map_sketch.getPixel(a, 1 + b + 2 * map_height - 1);
				}
